...
| `knowledge-reingest-nightly.json` | Schedule, `30 3 * * *` (instance timezone) | Enqueues `knowledge-reingest {corpus: homelab-notes}` on jobs-mcp (`budget_cap: 0`, day-keyed `idempotency_key` so a re-fire is a replay) using `$env.JOBS_MCP_BEARER_TOKEN`. Imported INACTIVE: arming it puts the jobs-mcp bearer (the whole v1 tool surface) into the n8n env — an explicit operator decision with two lighter alternatives, see `docs/runbooks/knowledge-mcp.md` "Scheduled freshness". |
