...
| workspace spend-limit error shape | a 429 without `retry-after`, a 403 `billing_error`, or a 400 — the code maps the first two to `E_LANE_UNAVAILABLE` and reads a 400's message for spend-limit markers | **✅ MEASURED 2026-09-14: a 400** `invalid_request_error` whose message says the workspace API usage limit is reached and names the resume time (the first of next month, 00:00 UTC). The 400 message-marker path mapped it: `503 E_LANE_UNAVAILABLE`, `Retry-After: 3600`, reservation released at $0, `gateway_upstream_errors_total{kind="spend_limit"}` +1, lane down with a one-hour `cooling_until`. **`count_tokens` is gated too** (the same 400; it fell back to the byte heuristic first). The resume time in the message is NOT parsed — the lane cools one hour, then half-opens: kept deliberately, because a raised limit then restores within the hour instead of the lane staying down until the month turns. The provider text appears only in the `evt=count_tokens_fallback` log line; `evt=lane_down` carries just the reason. **Enforcement lag: one call** — with the workspace already at $1.0009, the next 8000-token Opus call was still admitted; the one after was refused. Total spend for the proof: $1.2010. |
| is `models.list()` gated by the workspace spend limit | unknown; assumed NOT (hence the 0.1.1 probe fix) — if it is, the probe marks the lane down by itself | **✅ NOT gated, MEASURED 2026-09-14**: with the workspace refusing generations, the idle probe succeeded at 14:01:13Z (`last_ok` in `/ledger/lanes` advanced) while `up` stayed false and `cooling_until` stayed put — the 0.1.1 fix proven live: under 0.1.0 that probe would have flipped the lane up, silenced `GatewayLaneDown`, and sent traffic back into the limit. When reading this, remember the probe loop picks its interval on waking: the first probe after a lane goes down can take up to the 5-minute up-interval, not 60 s. |
| §13-9 max-`max_tokens` Opus under the 300 s read timeout | unmeasured; the trip uses 8000 with `X-Gateway-Timeout-S: 600` to stay clear | **✅ MEASURED 2026-09-14**: six 8000-token Opus completions took 121–124 s each (≈ 65 tokens/s), so a 16000-token call is ≈ 245 s — under the 300 s read timeout with little margin; long Opus calls should send `X-Gateway-Timeout-S` |
